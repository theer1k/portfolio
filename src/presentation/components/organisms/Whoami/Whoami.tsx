...
export const Whoami = () => {
  ...
  const description = [
    `I am a professional with experience in web development, mobile applications, and cloud solutions, having worked as a software engineer for the past five years. <p class="mb-6"></p> I have contributed to large-scale projects catering to thousands of users for esteemed companies like Nike and Centauro within the retail/e-commerce sector.`,
    ...
    <>
      ...
      >
        ...
        <div
          ...
          className="bottom-0 hidden h-80 w-full flex-col gap-4 overflow-hidden py-6 lg:absolute lg:flex"
        >
          <div
            style={{ opacity: 0.7 }}
          ...
          <h2
            ...
            } z-10 overflow-x-hidden px-10 pb-4 pt-6 text-xl font-bold text-green-500`}
          ></h2>
          <span
      ...
      <div
        ...
        className={`${allowScrolling ? 'visible' : 'hidden'} bottom-0 flex size-full min-h-35rem flex-col gap-4 overflow-x-hidden border-b-4 border-green-500 py-6 lg:hidden`}
      >
        <h2
          ref={elementToAppendPreDescriptionMobile}
          className={`${
            inViewDescriptionMobile ? 'block' : 'hidden'
          } z-10 overflow-x-hidden px-10 pb-4 pt-6 text-center text-2xl font-bold text-green-500`}
        ></h2>
        <span
          ref={elementToAppendDescriptionMobile}
          className={`${
            inViewDescriptionMobile ? 'block' : 'hidden'
          } z-10 overflow-x-hidden px-10 pb-6 text-center text-xl text-green-500 shadow-green-500`}
        ></span>
      </div>
    </>
  );
